...
public class Item_HeadOfTheDuke extends Item {
	public Item_HeadOfTheDuke(int grade) {
		super();
		this.grade = grade;
		try {
			image = ImageIO.read(new File("data/image/item/item19.gif"));
		} catch (IOException e) {
		}
		//setTtl((grade+1) * 70);
		...
		String tmptext = "<html>";
		tmptext += "Терновый венец "
				+ "<br>Пассивно:<br> +"+tmpValue+" int всем вашим героям";
		setToolTipText(tmptext);

		setName("Терновый венец");

		LHoH.gameScreen.heroAbilityStock.addAbility(new HeroAbility_CircletOfWisdom(tmpValue));
		// ttl=6;
	}
